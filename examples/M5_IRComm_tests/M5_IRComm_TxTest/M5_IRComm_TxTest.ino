...
/*
 * 
 * Testing if the m5 can reconfigure the IR comm
 * board and send messages at specific time 
 * intervals.
 * 
 */
...
void setup() {
  ...
  // Populate the tx settings struct
  // from the ir board itself
  getTxSettings();
  delay(5);

  // modify the period
  // note, keeping default tx_repeat 3
  tx_settings.tx_period_max = 1000;
  setTxSettings();
  delay(5);

  // We only need to set the string to
  // transmit once.  The board will then
  // transmit it by the settings above
  setIRMessage("test", 4);

  Serial.println("Transmitting Unit Test");
