...
#pragma pack(1)
// This struct needs to be identical to the one
// defined in the tracker.cpp file.
typedef struct upload {
	float x;
	float y;
	float probability;
	unsigned long observed_time;
	uint8_t hop_count;
	uint8_t confirmed_times;
} upload_t;

upload_t upload_data;
...
// How frequently to upload new data
// back to the tracking system to log.
// You might want to make this event
// based, i.e. when the robot adds a
// new obstacle to it's map.
unsigned long upload_update_ts;
#define UPLOAD_UPDATE_MS 100
...
//        Serial.print("Read packets: "); Serial.println(n_packets);
  ...
  if ( millis() - upload_update_ts > UPLOAD_UPDATE_MS ) {
    upload_update_ts = millis();

    // are we still connected to the server?
    if ( client.connected() ) {

      // This is just some random test data.
      memset( &upload_data, 0, sizeof( upload_data ) );
      upload_data.x = random(0,1000);
      upload_data.y = random(0,1000);
      upload_data.probability = (float)random(0,1000);
      upload_data.observed_time = millis();
      upload_data.hop_count = random(0,1000);
      upload_data.confirmed_times = random(0,1000);

      int err = client.write( (uint8_t*)&upload_data, sizeof( upload_data ) );

      if ( err == sizeof( upload_data ) ) {
